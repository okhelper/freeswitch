...
/*
code add by okhelper,at 2017-06-27
*/
static const char *global_cf = "default_odbc.conf";
switch_cache_db_handle_t *get_db_handle(const char* odbc_dsn)
{
	switch_cache_db_handle_t *dbh = NULL;
	
	if (!zstr(odbc_dsn)) 
	{
		if (switch_cache_db_get_db_handle_dsn(&dbh, odbc_dsn) != SWITCH_STATUS_SUCCESS) 
		{
...
switch_cache_db_handle_t *get_db_handle_x()
{
	switch_cache_db_handle_t *dbh = NULL;
	switch_xml_t cfg, xml, settings, param;
	if(!zstr(odbc_dsn))
	{
		//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CONSOLE, "---------odbc_dsn_ready:%s\n", odbc_dsn);
		return get_db_handle(odbc_dsn);
	}else{
...
SWITCH_STANDARD_API(originate_function)
{
	...
	long lg_time_now = 0;
	// const char* channel_odbc_dsn = NULL;
	const char* call_serial = NULL;
	switch_cache_db_handle_t *dbh = NULL;
	char resbuf[256] = "0";
	if (zstr(cmd)) {
		stream->write_function(stream, "-USAGE: %s\n", ORIGINATE_SYNTAX);
	...
	if(!dbh)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_NOTICE,"Callserial Database Error\n");
	}else{
		
		time_now = switch_micro_time_now();
		lg_time_now = (long)time_now;
		char *param_tmp = strdup(mycmd);
		char *param_array[10] = { 0 }; 
		int param_size = switch_separate_string(param_tmp ,'/',param_array,(sizeof(param_array) / sizeof(param_array[0])));
		const char* request_callserial_str = switch_mprintf("%ld/%s",lg_time_now,param_array[param_size-1]);
		char* serial_sql = switch_mprintf("insert into callcenter.business_callserial (serial_channel) values ('%s')",request_callserial_str);
		...
		call_serial = switch_mprintf("%s",resbuf);
		free(serial_sql);
		serial_sql = 0;	
	}

	if(!zstr(call_serial)){
		switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE,"call_serial:%s\n",call_serial);
		aleg = switch_mprintf("{call_serial=%s}%s",resbuf,argv[i++]);
	}else{
		aleg = argv[i++];
	}
	// aleg = argv[i++];
	...
	if(dbh != NULL) switch_cache_db_release_db_handle(&dbh);
	//switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_NOTICE,"-----originate_function[aleg:%s]-----\n",aleg);
