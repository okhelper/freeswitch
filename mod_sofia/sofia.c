						  ...
						  tagi_t tags[])
{
	...
	//switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CONSOLE, "---------profile->odbc_dsn---- %s\n", profile->odbc_dsn);
	...
	if (event == nua_i_invite && !sofia_private) {
		...
		}else 
		{
			/*
				add by okhelper
				2016-12-02
				remark:using Phone call out and freeswitch recv callout's inbound event,
				mod_sofia create session by ext_number bind channel_uuid
			*/
			switch_bool_t session_flag = SWITCH_FALSE;
			if (sip->sip_from && profile->name) 
			{
				
				// !strcasecmp(class, "PRESENCE_IN")
				const char *call_dir = profile->name;
				// switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE,"profile->name[%s],%d\n",call_dir,strcasecmp(call_dir,"internal"));

				if(!strcasecmp(call_dir,"internal"))
				{
					const char *ext_number = sip->sip_from->a_url->url_user;
					if(ext_number && dbh)
					{
						
						int uuid_exist_flag;
						char*_sql = switch_mprintf("select count(1) as uuid_exist_flag from callcenter.business_uuid where extension = '%s'",ext_number);
						char resbuf[256] = "0";
						switch_cache_db_execute_sql2str(dbh, _sql, resbuf, sizeof(resbuf), NULL);
						
						uuid_exist_flag =  0;
						uuid_exist_flag = atoi(resbuf);
						if(uuid_exist_flag == 1)
						{
							_sql = switch_mprintf("select channel_uuid from callcenter.business_uuid where extension = '%s'",ext_number);
							memset(&resbuf,0x00,sizeof(resbuf));
							switch_cache_db_execute_sql2str(dbh, _sql, resbuf, sizeof(resbuf), NULL);
							if( strlen(resbuf) > 1 )
							{
								switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE,"extension [%s] bind channel uuid [%s]\n",ext_number,resbuf);
								session_flag = SWITCH_TRUE;
								session = switch_core_session_request_uuid(sofia_endpoint_interface, SWITCH_CALL_DIRECTION_INBOUND, SOF_NONE, NULL, resbuf);
							}
						}else{
							char new_uuid[128] = "0";
							switch_uuid_t uuid;
							switch_uuid_get(&uuid);
							switch_uuid_format(new_uuid, &uuid);
							switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE,"extension [%s] create_new_uuid [%s]\n",ext_number,new_uuid);
							_sql = switch_mprintf("insert into callcenter.business_uuid (channel_uuid,extension) values ('%s','%s')",new_uuid,ext_number);
							switch_cache_db_execute_sql(dbh,_sql,NULL);
							session_flag = SWITCH_TRUE;
							session = switch_core_session_request_uuid(sofia_endpoint_interface, SWITCH_CALL_DIRECTION_INBOUND, SOF_NONE, NULL, new_uuid);
						}
					
					}
				}
			}
			if(session_flag == SWITCH_FALSE) session = switch_core_session_request(sofia_endpoint_interface, SWITCH_CALL_DIRECTION_INBOUND, SOF_NONE, NULL);
		}

		if (session) {
			const char *channel_name = NULL,*request_callserial_str = NULL;
			switch_time_t time_now = 0;
			long lg_time_now = 0;
			char resbuf[256] = "0";
			char* serial_sql = NULL;
			...
			//switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE,"channel_name[%s]",channel_name);
			sofia_glue_attach_private(session, profile, tech_pvt, channel_name);
			time_now = switch_micro_time_now();
			lg_time_now = (long)time_now;
			request_callserial_str = switch_mprintf("%ld_%s",lg_time_now,channel_name);
			serial_sql = switch_mprintf("insert into callcenter.business_callserial (serial_channel) values ('%s')",request_callserial_str);
			if(dbh) 
			{
				switch_cache_db_execute_sql(dbh,serial_sql,NULL);
				
				serial_sql = switch_mprintf("select call_serial from callcenter.business_callserial where serial_channel= '%s'",request_callserial_str);
				switch_cache_db_execute_sql2str(dbh,serial_sql,resbuf,sizeof(resbuf),NULL);
				switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_NOTICE,"request_callserial:%s-->%s\n",request_callserial_str,resbuf);
				switch_channel_set_variable(tech_pvt->channel,"call_serial",resbuf);
				free(serial_sql);
				serial_sql = 0;
  ...
  state_process:
	...
	case nua_callstate_proceeding:
		...
		case 181:
			{
				 switch_event_t *event;
                if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM,"ringing::transfer") == SWITCH_STATUS_SUCCESS)
                {
                    switch_channel_t *channel = switch_core_session_get_channel(session);
                    switch_channel_event_set_data(channel, event);
                    switch_event_fire(&event);
