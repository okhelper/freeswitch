...
void sofia_glue_execute_sql(sofia_profile_t *profile, char **sqlp, switch_bool_t sql_already_dynamic)
{
	...
	if(!dbh)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CONSOLE, "get sofia profile odbc failure,using sofia default sql engine\n");
		switch_sql_queue_manager_push(profile->qm, sql, 1, !sql_already_dynamic);
	}else{
		switch_cache_db_execute_sql(dbh,sql,NULL);
		//switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE,"----sofia_glue_execute_sql:%s\n",sql);
		switch_cache_db_release_db_handle(&dbh);
	}
...
void sofia_glue_execute_sql_now(sofia_profile_t *profile, char **sqlp, switch_bool_t sql_already_dynamic)
{
	...
	if(!dbh)
	{
		...
		switch_cache_db_execute_sql(dbh,sql,NULL);
		//switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE,"----sofia_glue_execute_sql_now:%s\n",sql);
		switch_cache_db_release_db_handle(&dbh);
	}
...
void sofia_glue_actually_execute_sql_trans(sofia_profile_t *profile, char *sql, switch_mutex_t *mutex)
{
	...
	switch_log_printf(SWITCH_CHANNEL_LOG,SWITCH_LOG_CONSOLE,"-----------sofia_glue_get_db_handle-------------\n");
